...
def delete(table):
    if table=='User':
        ...
        df2 = pd.DataFrame(new_result, columns=['Fname', 'Lname', 'Mail', 'Gender', 'Age', 'Phone', 'Uname'])
        with st.expander("Updated data"):
            st.dataframe(df2)

    elif table=='Feedback':
        result = view_all_feedback()
        df = pd.DataFrame(result, columns=['User','Feedback'])
        with st.expander("Current data"):
            st.dataframe(df)
        ...
        if st.button("Delete Feedback"):
            delete_feedback(selected_user)
            st.success("Feedback has been deleted successfully")
        new_result = view_all_feedback()
        df2 = pd.DataFrame(new_result, columns=['User','Feedback'])
        with st.expander("Updated data"):
            st.dataframe(df2)
'''
