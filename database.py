...
def create_table(table):
    if table=='User':
        c.execute('CREATE TABLE IF NOT EXISTS User(Fname TEXT, Lname TEXT, Mail TEXT, Gender TEXT, Age TEXT, Phone TEXT, Uname TEXT)')
    elif table=='Profile':
        c.execute('CREATE TABLE IF NOT EXISTS Profile(Uname TEXT, Gender TEXT, Age TEXT, Height TEXT, Religion TEXT, Job TEXT, Nationality TEXT, Mothertongue TEXT)')
    elif table=='Preference':
        c.execute('CREATE TABLE IF NOT EXISTS Preference(Uname TEXT, Gender TEXT, Age TEXT, Height TEXT, Religion TEXT, Job TEXT, Nationality TEXT, Mothertongue TEXT)')
    elif table=='Feedback':
        c.execute('CREATE TABLE IF NOT EXISTS Feed(Uname TEXT, Feedback TEXT)')
    '''PHOTO!!!!!!!!
...
def add_user_account(Fname, Lname, Mail, Gender, Age, Phone, Uname):
        c.execute('INSERT INTO User(Fname, Lname, Mail, Gender, Age, Phone, Uname) VALUES (%s,%s,%s,%s,%s,%s,%s)',
                  (Fname, Lname, Mail, Gender, Age, Phone, Uname))
        mydb.commit()

def add_profile_info(Uname, Gender, Age, Height, Religion, Job, Nationality, Mothertongue):
        c.execute('INSERT INTO wallet(Uname, Gender, Age, Height, Religion, Job, Nationality, Mothertongue) VALUES (%s,%s,%s,%s,%s,%s,%s,%s)',
                  (Uname, Gender, Age, Height, Religion, Job, Nationality, Mothertongue))
        mydb.commit()

def add_pref(Uname, Gender, Age, Height, Religion, Job, Nationality, Mothertongue):
        c.execute('INSERT INTO Prefrence(Uname, Gender, Age, Height, Religion, Job, Nationality, Mothertongue) VALUES (%s,%s,%s,%s,%s,%s,%s,%s)',
                  (Uname, Gender, Age, Height, Religion, Job, Nationality, Mothertongue))
        mydb.commit()


def add_feedback(Uname, Feedback):
        c.execute('INSERT INTO Feedback(Uname, Feedback) VALUES (%s,%s)',
                  (Uname, Feedback))
        mydb.commit()
...
def view_only_user_account():
    ...
    data = c.fetchall()
    return data
def view_only_profile():
    c.execute('SELECT Uname FROM Profile')
    data = c.fetchall()
    return data
def view_only_pref():
    c.execute('SELECT Uname FROM Prefrences')
    data = c.fetchall()
    return data
    '''
...
def aggregate():
    c.execute('SELECT COUNT(Feedback) FROM Feedback group by Feedback;')
    data = c.fetchall()
    return data

def aggregate_2():
    c.execute('SELECT AVERAGE(Feedback) FROM Feedback;')
    data = c.fetchall()
    return data
'''
def aggregate_3():
